...
class StorageService {
  ...
  Future<void> saveDraftTransactions(List<Transaction> draftTransactions) async {
    final jsonList = draftTransactions.map((transaction) => transaction.toJson()).toList();
    final jsonString = jsonEncode(jsonList);
  ...
  Future<List<EnvelopeBudget>> loadEnvelopeBudgets() async {
    ...
    final jsonList = jsonDecode(jsonString) as List;
    return jsonList.map((json) => EnvelopeBudget.fromJson(json)).toList();
  }
  ...
  Future<List<ZeroBasedBudget>> loadZeroBasedBudgets() async {
    ...
    final jsonList = jsonDecode(jsonString) as List;
    return jsonList.map((json) => ZeroBasedBudget.fromJson(json)).toList();
  }
}
