# Core Utils 工具类文档

## 概述

`core/utils/` 是Flux Ledger的工具函数集合，提供通用的辅助功能和跨模块共享的实用工具，支持应用的开发效率和代码质量。

**文件统计**: 10个Dart文件，实现11个核心工具模块

## 架构定位

### 层级关系
```
各个功能模块 (features/screens/core)  # 工具使用者
    ↓ (工具调用)
core/utils/                         # 🔵 当前层级 - 工具集合
    ↓ (基础功能)
Flutter SDK + Dart语言特性          # 基础依赖
    ↓ (平台服务)
iOS/Android/Web平台                 # 平台适配
```

### 职责边界
- ✅ **工具函数**: 提供纯函数和通用工具
- ✅ **跨模块共享**: 被多个模块共同使用的功能
- ✅ **平台适配**: 处理平台差异和兼容性
- ✅ **开发辅助**: 提升开发效率的辅助功能
- ❌ **业务逻辑**: 不包含具体的业务规则
- ❌ **状态管理**: 不负责数据状态维护

## 工具分类

### 1. 日志和调试工具 (3个文件)

#### Logger (`logger.dart`)
**职责**: 结构化日志记录系统

**核心功能**:
- 多级别日志记录 (Debug, Info, Warning, Error)
- 结构化日志格式
- 日志过滤和搜索
- 性能监控集成

**关联关系**:
- **依赖**: Dart的`developer`库
- **被依赖**: 所有需要日志记录的模块
- **级联影响**: 应用的调试和监控能力

#### FluxLogger (`flux_logger.dart`)
**职责**: Flux Ledger专用轻量日志桩

**核心功能**:
- 业务日志和错误日志的统一格式
- 轻量级实现，避免重依赖
- 后续可平滑替换为完整日志系统

**关联关系**:
- **依赖**: `logger.dart` (基础日志功能)
- **被依赖**: `main_flux.dart` (应用入口日志)
- **级联影响**: 应用启动和运行时的日志记录

#### DebugModeManager (`debug_mode_manager.dart`)
**职责**: 调试模式管理和配置

**核心功能**:
- 调试开关管理
- 开发者选项控制
- 测试数据生成
- 性能监控开关

**关联关系**:
- **依赖**: SharedPreferences (配置存储)
- **被依赖**: 开发者模式页面
- **级联影响**: 开发和测试体验

### 2. 数据处理工具 (3个文件)

#### CurrencyFormatter (`currency_formatter.dart`)
**职责**: 货币格式化和显示工具

**核心功能**:
- 多货币格式支持
- 本地化货币显示
- 金额精确计算
- 汇率转换辅助

**关联关系**:
- **依赖**: Dart的`intl`库
- **被依赖**: 所有金额显示组件
- **级联影响**: 财务数据的显示一致性

#### AiDateParser (`ai_date_parser.dart`)
**职责**: AI增强的日期解析工具

**核心功能**:
- 自然语言日期解析
- 相对日期计算
- 多格式日期识别
- 时区处理

**关联关系**:
- **依赖**: AI服务 (可选)
- **被依赖**: 交易录入和日期选择组件
- **级联影响**: 日期输入的用户体验

#### SalaryFormValidators (`salary_form_validators.dart`)
**职责**: 工资表单验证工具

**核心功能**:
- 工资数据格式验证
- 业务规则检查
- 错误提示生成
- 数据完整性验证

**关联关系**:
- **依赖**: 工资相关的业务规则
- **被依赖**: 工资设置表单
- **级联影响**: 工资数据录入的准确性

### 3. 用户交互工具 (3个文件)

#### NotificationManager (`notification_manager.dart`)
**职责**: 通知管理和调度系统

**核心功能**:
- 本地通知调度
- 通知权限管理
- 通知模板管理
- 通知历史记录

**关联关系**:
- **依赖**: 平台通知API
- **被依赖**: 提醒和通知功能
- **级联影响**: 用户提醒功能的有效性

#### UnifiedNotifications (`unified_notifications.dart`)
**职责**: 统一通知系统

**核心功能**:
- 多渠道通知集成
- 通知优先级管理
- 通知去重和合并
- 通知统计分析

**关联关系**:
- **依赖**: `NotificationManager`
- **被依赖**: 全局通知需求
- **级联影响**: 通知系统的统一性和效率

#### PerformanceMonitor (`performance_monitor.dart`)
**职责**: 性能监控和分析工具

**核心功能**:
- UI渲染性能监控
- 内存使用跟踪
- 方法执行时间测量
- 性能指标收集

**关联关系**:
- **依赖**: Dart的性能分析API
- **被依赖**: 性能优化和调试
- **级联影响**: 应用性能的持续优化

### 4. 验证和异常处理 (1个文件)

#### VerificationExceptions (`verification_exceptions.dart`)
**职责**: 验证框架的异常定义和处理

**核心功能**:
- 验证异常类型定义
- 异常处理策略
- 错误信息格式化
- 异常链追踪

**关联关系**:
- **依赖**: Dart异常处理机制
- **被依赖**: `verifiers/` 验证框架
- **级联影响**: 验证过程的错误处理一致性

## 工具设计原则

### 1. 纯函数优先
大部分工具函数设计为纯函数，便于测试和复用。

### 2. 单例模式
需要状态管理的工具类使用单例模式。

### 3. 依赖注入
工具类通过构造函数注入依赖，便于测试。

### 4. 错误处理
完善的错误处理和异常传播机制。

### 5. 平台适配
考虑不同平台的差异和兼容性。

## 工具调用模式

### 静态工具方法
```dart
// 货币格式化
String formatted = CurrencyFormatter.formatAmount(1234.56, 'CNY');

// 日期解析
DateTime? date = AiDateParser.parseDateString('明天上午10点');
```

### 单例工具类
```dart
// 日志记录
Logger.instance.info('用户登录', {'userId': '123'});

// 性能监控
PerformanceMonitor.instance.startTiming('data_load');
```

### 扩展方法
```dart
// 字符串扩展
extension StringExtensions on String {
  String toCurrency() => CurrencyFormatter.formatAmount(double.parse(this), 'CNY');
}

// 日期扩展
extension DateTimeExtensions on DateTime {
  String toRelativeString() => AiDateParser.toRelativeString(this);
}
```

## 性能优化

### 1. 缓存机制
常用数据的内存缓存，减少重复计算。

### 2. 懒加载
工具类的延迟初始化，减少应用启动时间。

### 3. 异步处理
长时间操作的异步执行，避免UI阻塞。

### 4. 资源管理
及时释放资源，防止内存泄漏。

## 测试策略

### 单元测试
- 纯函数的输入输出测试
- 工具类的功能测试
- 边界条件和异常测试

### 集成测试
- 工具与平台的集成测试
- 多工具协同工作测试
- 性能基准测试

### 端到端测试
- 工具在完整流程中的表现测试
- 用户体验相关的工具测试

## 扩展开发

### 添加新的工具类
```dart
class NewUtility {
  static final NewUtility _instance = NewUtility._internal();

  factory NewUtility() => _instance;

  NewUtility._internal();

  ResultType performOperation(InputType input) {
    // 实现工具逻辑
  }
}
```

### 添加扩展方法
```dart
extension NewExtensions on ExistingType {
  ReturnType newMethod() {
    // 实现扩展逻辑
  }
}
```

## 使用指南

### 日志记录
```dart
// 基础日志
Logger.instance.info('操作成功', {'details': '额外信息'});

// 错误日志
Logger.instance.error('网络请求失败', error, stackTrace);

// 性能日志
PerformanceMonitor.instance.logTiming('api_call', duration);
```

### 货币处理
```dart
// 格式化显示
String displayAmount = CurrencyFormatter.formatAmount(1234.56, 'CNY');

// 计算处理
double result = CurrencyFormatter.calculateTax(10000, 0.03);
```

### 通知管理
```dart
// 调度通知
await NotificationManager.instance.scheduleNotification(
  title: '提醒',
  body: '记得记录支出',
  scheduledTime: DateTime.now().add(Duration(hours: 1)),
);
```

## 相关文档

- [Core包总文档](../README.md)
- [Logger使用指南](logger.dart)
- [性能监控最佳实践](performance_monitor.dart)



