...
sealed class AccountState {
  ...
  List<Account> get accounts =>
      this is AccountStateLoaded ? (this as AccountStateLoaded).accounts : [];
  String get errorMessage =>
      this is AccountStateError ? (this as AccountStateError).message : '';
}
...
class AccountNotifier extends StateNotifier<AccountState> {
  AccountNotifier(this._storageService) : super(const AccountStateInitial()) {
    loadAccounts();
  }
...
final activeAccountsProvider = Provider<List<Account>>((ref) {
  ...
  return state.isLoaded
      ? state.accounts.where((a) => a.status == AccountStatus.active).toList()
      : [];
});
...
final isAccountLoadingProvider =
    Provider<bool>((ref) => ref.watch(accountProvider).isLoading);
