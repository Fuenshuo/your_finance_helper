...
class AccountProvider with ChangeNotifier {
  ...
  Future<Transaction?> createAccountInitializationTransaction(
    ...
    final initTransaction = Transaction(
      ...
      date: initDate, // 使用创建日期前一天，避免计入当月收入
      notes: '账户初始余额设置',
      isAutoGenerated: true, // 标记为自动生成，不计入统计
    );

    await transactionProvider.addTransaction(initTransaction);

    // 账户的initialBalance保持为0，余额完全通过交易计算
    // 无需更新账户

    return initTransaction;
  }

  // 不再直接更新账户余额，而是通过交易来改变
  // 这个方法保留用于向后兼容，但不应该被调用
  @deprecated
