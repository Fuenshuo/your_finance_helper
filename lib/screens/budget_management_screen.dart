...
import 'envelope_budget_detail_screen.dart';
...
class _BudgetManagementScreenState extends State<BudgetManagementScreen>
  ...
  Widget _buildOverviewTab() {
    return Consumer2<BudgetProvider, AccountProvider>(
              ...
              Expanded(
                child: StatCard(
                  ...
                  value: totalAllocated > 0 
                    ? context.formatPercentage((totalSpent / totalAllocated) * 100)
                    : '0%',
  ...
  // 当前零基预算卡片
  Widget _buildCurrentZeroBasedCard(ZeroBasedBudget budget) {
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  // 信封列表标题
  Widget _buildEnvelopeListHeader() {
    return Padding(
  ...
  // 信封卡片
  Widget _buildEnvelopeCard(EnvelopeBudget envelope) {
    return AppCard(
          ...
          children: [
            ...
            LinearProgressIndicator(
              ...
              valueColor: AlwaysStoppedAnimation<Color>(
                Color(int.parse(_getBudgetStatusColor(envelope).replaceFirst('#', '0xff'))),
              ),
            ),
            SizedBox(height: context.spacing8),
  ...
  // 零基预算卡片
  Widget _buildZeroBasedCard(ZeroBasedBudget budget) {
    return AppCard(
      margin: EdgeInsets.only(bottom: context.spacing12),
      child: InkWell(
  ...
  void _navigateToZeroBasedDetail(ZeroBasedBudget budget) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('零基预算详情页面开发中...')),
    );
  }
}
