...
class _TransactionManagementScreenState extends State<TransactionManagementScreen>
  ...
  Widget _buildOverviewTab() {
    return Consumer3<TransactionProvider, AccountProvider, BudgetProvider>(
      builder: (context, transactionProvider, accountProvider, budgetProvider, child) {
        ...
        final thisMonthTransactions = transactions.where((t) => 
          t.date.isAfter(thisMonth) && t.status == TransactionStatus.confirmed
        ).toList();

        final lastMonthTransactions = transactions.where((t) => 
          t.date.isAfter(lastMonth) && t.date.isBefore(thisMonth) && t.status == TransactionStatus.confirmed
        ).toList();
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: StatCard(
                            title: '结余',
                            value: context.formatAmount(thisMonthIncome - thisMonthExpense),
                            valueColor: thisMonthIncome - thisMonthExpense >= 0 
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ...transactions
                        ...
                        .take(5)
                        .map((transaction) => _buildTransactionListItem(transaction, accounts))
                        .toList(),
            ...
            return AppCard(
              ...
              child: ListTile(
                leading: CircleAvatar(
                  backgroundColor: _getTransactionTypeColor(draft.type).withOpacity(0.1),
                  child: Icon(
                    _getTransactionTypeIcon(draft.type),
                    color: _getTransactionTypeColor(draft.type),
                  ),
                ),
                title: Text(draft.description),
                subtitle: Text(
                  '${draft.type.displayName} • ${draft.date.toString().split(' ')[0]}',
                ),
                trailing: Text(
  ...
  // 构建交易列表项
  Widget _buildTransactionListItem(Transaction transaction, List<Account> accounts) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 16,
            backgroundColor: _getTransactionTypeColor(transaction.type).withOpacity(0.1),
            child: Icon(
