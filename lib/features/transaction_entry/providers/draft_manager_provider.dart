...
class DraftManagerNotifier extends StateNotifier<DraftManagerState> {
  ...
  Future<void> saveDraft(DraftTransaction draft) async {
    try {
      ...
      state = state.copyWith(error: '保存草稿失败: $e');
    }
  }
  ...
  Future<void> deleteDraft(DraftTransaction draft) async {
    try {
      ...
      state = state.copyWith(error: '删除草稿失败: $e');
    }
  }
  ...
  Future<void> clearAllDrafts() async {
    try {
      ...
      state = state.copyWith(error: '清空草稿失败: $e');
    }
  }
  ...
  /// 检查是否有未保存的草稿
  bool hasUnsavedDraft(DraftTransaction currentDraft) => state.savedDrafts.any(
        (saved) =>
